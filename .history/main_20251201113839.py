...
X=df[["EngineSize_L","ModelYear"]].fillna(df.mean(numeric_only=True))
...
print("Her fold uzre score : ",scores)
print("Orta R2",np.mean(scores))
...
df=pd.read_excel("houses_day1 (1) 1.xlsx")
X=df[["Area_m2","Rooms"]].fillna(df.mean(numeric_only=True))
df=df[["Price_AZN"]].fillna(df.median(numeric_only=True))
y=df["Price_AZN"]
...
Məqsəd: Data xətti düzülməyibsə belə, onu əyrilikləri ilə düzgün proqnozlaşdırmaq.
#endregion
