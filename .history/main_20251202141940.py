...
#  xüsusilə dərin öyrənmə (deep learning) və nevro şəbəkələri
...
y = torch.tensor([0, 0, 0, 1, 1, 1], dtype=torch.float32).view(-1, 1)  #0 ucuz,1 baha
...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        # Hidden layer: 2 giriş neyronu → 4 çıxış neyronu
        self.fc1 = nn.Linear(2, 4)
        # Output layer: 4 hidden neyron → 1 çıxış neyronu
        self.fc2 = nn.Linear(4, 1)

    def forward(self, x):
        # Hidden layer üçün aktivasiya funksiyası (ReLU)
        x = torch.relu(self.fc1(x))
        # Output layer üçün aktivasiya funksiyası (Sigmoid, 0-1 ehtimal)
        x = torch.sigmoid(self.fc2(x))
        return x

# Model nümunəsi yaradılır
model = Net()

# Loss funksiyası (Binary Cross Entropy)
criterion = nn.BCELoss()  # 0/1 target üçün uyğundur

# Optimizer (Adam) - ağırlıqları yeniləyəcək alqoritm
optimizer = optim.Adam(model.parameters(), lr=0.01)  # lr = learning rate

# Training loop (5000 epoch)
for epoch in range(5000):
    # 1️⃣ Forward pass: model inputu götürüb proqnoz verir
    pred = model(x)

    # 2️⃣ Loss hesablanır: pred ilə real y müqayisə olunur
    loss = criterion(pred, y)

    # 3️⃣ Gradientləri sıfırla (əvvəlki iterasiyadan qalmasın)
    optimizer.zero_grad()

    # 4️⃣ Backpropagation: gradientlər hesablanır
    loss.backward()

    # 5️⃣ Ağırlıqları yenilə
    optimizer.step()

# Test nümunəsi üçün proqnoz
test = torch.tensor([[105, 3]], dtype=torch.float32)
# model(test).item() → test üçün çıxış ehtimalı (0-1 aralığında)
print(f"Prediction: {model(test).item()}")
