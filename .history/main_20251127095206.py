...
# Buradakı 2 → λ (lambda) adlanır.

# Poisson paylanmasında λ hadisələrin orta sayı (mean) deməkdir.

# | Parametr    | İzahı                                                                                        |
# | ----------- | -------------------------------------------------------------------------------------------- |
# | `loc=4`     | Normal paylanmanın **ortalama (mean/μ)** dəyəri. Burada **4** deməkdir.                      |
# | `scale=1`   | **Standart sapma (σ)**. Yəni dəyərlərin ortalama ətrafında nə qədər yayıldığı. Burada 1-dir.(std yeni on cox sapma 1 olacaq) |
# | `size=1000` | **Neçə ədəd** yaratmaq lazım. Burada 1000 ədəd təsadüfi qiymət yaranacaq.                    |



#Her Paylanmaya numune yaz nezeri \


houses=pd.read_excel('houses_day.xlsx')
# house_room_count=houses['Rooms']
...
possion_data=np.random.poisson(2,size=1000)
#
# plt.hist(possion_data,bins=range(0,10),color='salmon',edgecolor='black')
...
# p-value	Nəticə
# p > 0.05	Normal Distribution qəbul edilir (H0 rədd olunmur)
# p ≤ 0.05	Normal deyil (H0 rədd olunur)


# p-value → “Bu nəticə təsadüfən baş verə bilərmi?” sorusunun cavabı (tam basa dus sonradan)

# stats → bu sorğunu soruşmaq üçün istifadə etdiyimiz statistik funksiyalar və testlər (tam basa dus sonradan)



#bu dersin ikinci hissesine bax

# uniform_data=np.random.uniform(low=1, high=6, size=1000)
# # ks_uniform=stats.kstest(uniform_data,'uniform',args=(1,5))
# # print(ks_uniform.pvalue,ks_uniform.statistic)




# observed_count=np.bincount(possion_data)
# expected_count=[poisson.pmf(i,2)*len(possion_data) for i in range(len(observed_count))]


# expected_count[-1]+=len(possion_data)-sum(expected_count)
# chi_stat,p_value=chisquare(f_obs=observed_count,f_exp=expected_count)
# # print(chi_stat)
# # print(p_value)


# #skew 0 a chox yaxindirsa demeli bu normal paylanmadir
# # skew < 0 - o zaman sola meyillidir
# # skew > 0 - o zaman saga meyillidir
# print("Skewness with Normal data")
# print(stats.skew(normal_data))


# print("Skewness with Uniform data")
# print(stats.skew(uniform_data))
