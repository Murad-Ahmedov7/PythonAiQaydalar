...
# Variance (Dispersiya) – məlumatların orta dəyərdən(meanden) nə qədər uzaqlaşdığını ölçən statistik göstəricidir.

# Std (Standart sapma) → np.std() → variance-in kvadrat kökü
...
# 5️⃣ Outlier (Sıradan çıxan dəyərlər)
...
# 6️⃣ Fonksiyaların qısa müqayisəsi
# Funksiya	Mənası
# np.mean()	Orta
# np.median()	Median
# mode()	Ən çox təkrarlanan
# variance()	Yayılma (kvadrat)
# np.std()	Tipik sapma
# df.describe()	Statistik xülasə
# df.quantile()	Q1, Q2, Q3
# 7️⃣ NumPy vs Pandas diferensiyası
# Nüans	NumPy	Pandas
# Tip	Homogen	Heterogen
# Struktur	Array	Cədvəl
# İstifadə	Hesablama	Analiz
# 8️⃣ Variance – Std – Outlier fərqi

# Variance: Orta dəyərdən kvadrat fərqlərin ortalaması

# Std: Variance-in kökü (həqiqi ölçü vahidində)

# Outlier: Normadan çox kənarda olan dəyərlər (IQR və ya Std ilə tapılır
