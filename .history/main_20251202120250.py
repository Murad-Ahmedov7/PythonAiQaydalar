...
# model=LogisticRegression(max_iter=200)  #max_iter = “model ən çox bu qədər iterasiya işləyə bilər” → amma əgər daha tez öyrənsə, dayanacaq.
# model.fit(X_train,y_train)


# # model.predict_proba(X_test) → test nümunələrinin hər biri üçün bütün siniflərin ehtimallarını qaytarır
# # Binary classification üçün hər nümunə üçün 2 dəyər qaytarır: [prob_class_0, prob_class_1]
# # Məsələn: [[0.65, 0.35], [0.10, 0.90], [0.80, 0.20], ...]
# # [:,1] → bütün nümunələr üçün "1" sinifinə aid olan ehtimal sütununu götürür
# # Nəticədə pred_proba → 1 sinifinə aid ehtimalların siyahısı olur
# pred_proba = model.predict_proba(X_test)[:,1]

# # predict → birbaşa sinif proqnozu verir (0 və ya 1)
# pred = model.predict(X_test)

# # 1 sinifinə aid ehtimalları göstəririk
# print("pred prob : ", pred_proba)

# # 0/1 proqnoz nəticələrini göstəririk
# print("pred : ", pred)
...
# # confusion_matrix(y_test, pred) → modelin proqnozları ilə əsl (true) sinifləri müqayisə edərək
# # 2x2 (binary classification) matris yaradır:
# # [[True Negative (TN), False Positive (FP)],
# #  [False Negative (FN), True Positive (TP)]]
# #
# # İzah:
# # TN → Model 0 deyib, əslində də 0
# # FP → Model 1 deyib, əslində 0
# # FN → Model 0 deyib, əslində 1
# # TP → Model 1 deyib, əslində 1
# #
# # Məsələn:
# # [[50, 5],
# #  [10, 35]]
# # TN=50, FP=5, FN=10, TP=35
# # Bu matrix modelin hansı nümunələri düzgün, hansıları səhv proqnoz etdiyini göstərir
# #
# print(confusion_matrix(y_test, pred))  #→ matrisi çap edir


# # print(classification_report(y_test,pred)) bu ne edir???????????????????????


# #r2score ile accuracy ferqi nedi?????
...
# 🔹 X → giriş nümunələri (AND üçün)
# 4 nümunə var: [0,0], [0,1], [1,0], [1,1]
X = np.array([
    [0,0], 
    [0,1],
    [1,0],
...
# 🔹 y → çıxışlar (AND əməliatı nəticəsi)
# AND əməliatı: yalnız hər iki giriş 1 olduqda çıxış 1
y = np.array([0,0,0,1]) 

# 🔹 w → çəkilər (weights), random dəyərlə başlayırıq
w = np.random.rand(2)
print("Initial weights:", w)

# 🔹 b → bias, başlanğıcda 0
b = 0.0

# Step funksiyası sadə bir “sigmoid simulyasiyası” kimi düşünülə bilər:
# - Çıxış yalnız 0 və 1 olur
# - Z >= 0 → 1, Z < 0 → 0
# - Sigmoid funksiyası isə 0–1 arası ehtimal verir, daha yumşaq və diferensialdır

def step(z):
    return 1 if z >= 0 else 0

# 🔹 lr → learning rate, modelin çəkiləri nə qədər sürətlə yeniləyəcəyini göstərir
lr = 0.1

# 🔹 epochs → neçə dəfə bütün nümunələr üzərində təlim keçiriləcək
epochs = 10

for epoch in range(epochs):
    for xi, yi in zip(X, y):
        z = np.dot(xi, w) + b  # wx + b → linear toplam
        y_hat = step(z)         # aktivasiya funksiyası ilə 0/1 çıxış
        error = yi - y_hat      # səhv: doğru - proqnoz

        # Çəkilərin və biasın yenilənməsi
        w += lr * error * xi
        b += lr * error

        # Təlim zamanı nəticələrin çapı
        print(f"input = {xi} , real = {yi}  pred = {y_hat}  error = {error}  w = {w}  b = {b}")
...
# 🔹 Kodda random çəkilər
# w = np.random.rand(2)
# b = 0.0


# Bu cəmi bir dəfə icra olunur → model öyrənməyə bu random çəkilərdən başlayır

# Sonra təlim dövründə (for epoch in range(epochs): ...):


# for epoch in range(epochs):        # 1. for → 10 dəfə təlim (epochs = 10)
#     for xi, yi in zip(X, y):       # 2. for → hər epoch-da 4 nümunəni nəzərdən keçirir



# 🔹 Forward Pass (İrəli keçid)
# z = np.dot(xi, w) + b  # wx + b → linear toplam
# y_hat = step(z)         # aktivasiya funksiyası ilə 0/1 çıxış


# Forward pass = girişdən çıxışa gedən yol

# xi → giriş nümunəsi

# w və b ilə linear toplama (wx + b) → z

# step(z) → aktivasiya funksiyası → neyronun proqnozu y_hat

# Sadə sözlə: giriş → linear toplam → aktivasiya → çıxış

# 🔹 Backward Pass (Geriyə yayılma)
# error = yi - y_hat
# w += lr * error * xi
# b += lr * error


# Backward pass = səhvə görə çəkiləri və bias-i yeniləmək

# error = yi - y_hat → səhvi hesabla

# w += lr * error * xi → çəkiləri düzəlt

# b += lr * error → bias-i düzəlt

# Sadə sözlə: çıxışdakı səhv → çəkilərə və bias-a tətbiq olunur → növbəti dəfə daha düzgün çıxış alınır
